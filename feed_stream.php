...
<title><? echo title();?> | Memes By<? echo $title_meme?></title>
  ...
  <div class="page_right">
	...
	$pripoj = mysql_query("SELECT * FROM users WHERE id = '$useridr'");
		while($row = mysql_fetch_object($pripoj))
			{
				echo'<div class="profile_menu_icon"><img src="images/user_profile.png" alt="'.$row->name.'" width="46" height="34" align="left" />Meme By '.$row->name.'</div>';
			}
		$user = $_GET['user'];
		$feed = $_GET['feed'];
...
$kolo = 1;
	$pripoj = mysql_query("SELECT * FROM images WHERE userid = '$useridr' LIMIT ".intval($pocet).",$po");
		while($row = mysql_fetch_object($pripoj))
			{
				...
				<div class="profile_img_item">
					 <div class="profile_img_hov"><div id="simple-target-'.$kolo.'"><a href="'.$row->id.'-'.$title.'">
		...
		if($url_page>1){echo'<div class="paginge_left"><a href="feed_stream.php?page='.$url_page_p.'&user='.$useridr.'"><img src="images/previous.png" alt="previous_meme"></a></div>';}
		if($max_stranek > $url_page){echo'<div class="paginge_right"><a href="feed_stream.php?page='.$url_page.'&user='.$useridr.'"><img src="images/next.png" alt="next_meme"></a></div>';}
		echo'</div>';
	?>
      </div>
    </div>
